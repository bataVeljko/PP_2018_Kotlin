...
class FourthActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        dugme.setOnClickListener({
            val intent = Intent(this@FourthActivity, ProbnaActivity::class.java)
    ...
    override fun onResume() {
        ...
        popuniRed("ponedeljak", casovi, trPonedeljak)
        popuniRed("utorak", casovi, trUtorak)
        popuniRed("sreda", casovi, trSreda)
        popuniRed("cetvrtak", casovi, trCetvrtak)
        popuniRed("petak", casovi, trPetak)
    }
    ...
    private fun proveriPreklapanje(cas: Predmet): Boolean {
        for(casGlobalna in izabraniCasovi.filter { casLokalna -> casLokalna.dan == cas.dan }){
            when {
                (cas.vreme_pocetka >= casGlobalna.vreme_pocetka) and (cas.vreme_pocetka < casGlobalna.vreme_kraja) -> return preklapanjePoruka()
                (casGlobalna.vreme_pocetka >= cas.vreme_pocetka) and (casGlobalna.vreme_pocetka < cas.vreme_kraja) -> return preklapanjePoruka()
                (cas.vreme_pocetka == casGlobalna.vreme_pocetka) and (cas.vreme_kraja == casGlobalna.vreme_kraja) -> return preklapanjePoruka()
                else -> {}
            }
        }
        return true
