...
class ThirdActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(grupe.size > 1)
            grupeAdapter.sort { s1, s2 ->  s1.compareTo(s2)}
        ...
        dugme.setOnClickListener {
            ...
            if(izabraneLokacije.contains("Jagiceva"))
                pomocniFiltriraniCasoviZaLokacije.addAll(filtriraniCasovi.filter { cas -> cas.getUcionica().startsWith("ЈАГ") })
            if(izabraneLokacije.contains("N"))
                pomocniFiltriraniCasoviZaLokacije.addAll(filtriraniCasovi.filter { cas -> cas.getUcionica().startsWith("Н") })
